...
from hyperapi.schemas.json_schema_meta_schema import (
...
def add_properties_to_inputs(
    ...
    if type:
        return form.inputs.append(
            FormInput(
                type=to_form_input_type(property["type"]),
                name=name,
                id=form.id + name,
...
def jsonschema_to_form(
    form_id: str, action: str, method: str, form_cls: type[Form], schema: dict
):
    validated_schema = CoreAndValidationSpecificationsMetaSchema.model_validate(schema)
    form = form_cls(id=form_id, action=action, method=method, inputs=[])
    for key, value in validated_schema.properties.items():
        add_properties_to_inputs(form, key, value)
    return form
...
@pytest.fixture()
def form():
    return jsonschema_to_form(
        "instance1", "/", "POST", Form, MuleSchema.model_json_schema()
    )
